...
def read_parameters():
    ...
    parser.add_argument("--rlim", type=float, default=1.2, help="""Threshold for the contrast value of cosmics
    ...
    parser.add_argument("--iter", type=int, default=4, help="""Number of iteration to be performed by the algorithms. 
        Usually 4-5 iterations are needed to converge to a stable solution. Default is 5.""")
    parser.add_argument("--replacebox", type=int, nargs=2, default=[5, 5], help="""Size of the subimage along x and 
        ...
        be symmetric.""")
    parser.add_argument("--radius", type=int, default=0, help="""Integer number of neighboring pixel used to increase 
        the boundaries of the detected cosmics. Should only be used if considered to be absolutely necessary.""")
    parser.add_argument("--gain", type=str, default='1.0', help="""Header keyword of the CCD gain or 
        alternatively the corresponding value as float number when the bias-subtracted image was not yet converted 
        to electron.""")
    parser.add_argument("--bias", type=float, default=0.0, help="""Optional subtraction of a bias level. 
        The default is 0.0""")
    parser.add_argument("--ext_out", action="store_true", default=False, help="""Flag to store the results in a single
        FITS file in extensions DATA, ERROR and MASK.""")
    parser.add_argument("--verbose", action="store_true", default=False, help="""Flag to print some progress 
...
def load_file(input_image, extension_arg=0, gain_arg=1.0, rdnoise_arg=1.0):
    try:
        ...
        print("Input FITS file could not be found. Please check file name and path.")
        sys.exit(1)
