...
class Image(object):
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, Image):
            ...
            # add error if contained in both
            if self._error is not None and other._error is not None:
                new_error = numpy.sqrt((self._error / other._data)**2 +
    ...
    def data(self, data):
        if data is None:
            ...
            if self._dim is None:
                ...
                raise RuntimeError(
                    "Incompatible dimensions for replacing Image data.")
    ...
    def mask(self, mask):
        if mask is None:
            ...
            if self._dim is None:
                ...
                raise RuntimeError(
                    "Incompatible dimensions for replacing Image mask.")
    ...
    def error(self, error):
        if error is None:
            ...
            if self._dim is None:
                ...
                raise RuntimeError(
                    "Incompatible dimensions for replacing Image error.")
    ...
    def replaceMaskMedian(self, box_x, box_y, replace_error=1e20):
        ...
        for m in range(len(y_cors)):
            ...
            range_y = numpy.clip(
                [y_cors[m] - delta_y, y_cors[m] + delta_y + 1], 0,
                self._dim[0] - 1).astype(numpy.uint16)
            range_x = (numpy.clip(
                [x_cors[m] - delta_x, x_cors[m] + delta_x + 1], 0,
                self._dim[1] - 1)).astype(numpy.uint16)
            # compute the masked median within the filter window and replace data
            select = self._mask[range_y[0]:range_y[1],
                                range_x[0]:range_x[1]] == 0
            out_data[y_cors[m], x_cors[m]] = numpy.median(
                self._data[range_y[0]:range_y[1],
    ...
    def rebin(self, bin_x, bin_y):
        ...
        if (self._dim[0] % bin_y) != 0 or (self._dim[1] % bin_x) != 0:
            raise RuntimeError(
                "Binning cannot be performed. Input dimensions are not a multiple of the requested"
                " binning.")
        ...
        new = numpy.sum(
            numpy.reshape(
                self._data,
                (self._dim[0], int(self._dim[1] / bin_x), int(bin_x))), 2)
        new2 = numpy.sum(
            numpy.reshape(new, (int(
                self._dim[0] / bin_y), int(bin_y), int(self._dim[1] / bin_x))),
            1)
        ...
        if self._error is not None:
            ...
            error_new2 = numpy.sqrt(
                numpy.sum(
                    numpy.reshape(error_new,
                                  (int(self._dim[0] / bin_y), int(bin_y),
        ...
        if self._mask is not None:
            ...
            mask_new = numpy.sum(
                numpy.reshape(
                    ...
                    (self._dim[0], int(self._dim[1] / bin_x), int(bin_x))), 2)
            mask_new2 = numpy.sum(
                numpy.reshape(mask_new, (int(self._dim[0] / bin_y), int(bin_y),
                                         int(self._dim[1] / bin_x))), 1)
    ...
    def convolve_gauss(self, sigma_x, sigma_y, mode='nearest', mask=False):
        ...
        if self._mask is not None and mask:
            ...
            gauss = ndimage.filters.gaussian_filter(self._data,
                                                    (sigma_y, sigma_x),
                                                    mode=mode)
            scale = ndimage.filters.gaussian_filter(
                (self._mask == False).astype('float32'), (sigma_y, sigma_x),
                mode=mode)
            new = gauss / scale
            self._data[self._mask] = mask_data
        else:
            new = ndimage.filters.gaussian_filter(self._data,
                                                  (sigma_y, sigma_x),
                                                  mode=mode)
        # create new Image object with the error and the mask unchanged and return
