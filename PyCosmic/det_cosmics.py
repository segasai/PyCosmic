...
def det_cosmics(data,
    ...
    if (gain != 1.0) and verbose:
        print(
            'Convert image from ADUs to electrons using a gain factor of %f' %
            (gain))
    img = img * gain
    ...
    if (rdnoise > 0.0) and verbose:
        print('A value of %f is used for the electron read-out noise.' %
              rdnoise)
    img_original.error = np.sqrt(
        (np.clip(img_original.data, a_min=0.0, a_max=None) + rdnoise**2))

    select = numpy.zeros(img.dim, dtype=bool)
    img_original.mask = np.zeros(img.dim, dtype=bool)
    img.mask = np.zeros(img.dim, dtype=bool)
    ...
    for i in range(iterations):
        ...
        select_neg = conv < 0
        conv.replace_subselect(select_neg,
                               data=0)  # replace all negative values with 0
        ...
        S_prime = S - S.medianImg(
            (5, 5))  # cleaning of the normalized Laplacian image

        # Perform additional clean using a 2D Gaussian smoothing kernel
        fine = out.convolve_gauss(
            sigma_x, sigma_y, mask=True)  # convolve image with a 2D Gaussian
        fine_norm = out / fine
        ...
        if verbose:
            ...
            print('Total number of detected cosmics: %i out of %i pixels' %
                  (int(det_pix), dim[0] * dim[1]))

        if i == iterations - 1:
            img_original.replace_subselect(select,
                                           mask=True)  # set the new mask
            if increase_radius > 0:
                mask_img = Image(data=img_original._mask)
                mask_new = mask_img.convolve(
                    kernel=numpy.ones((2 * increase_radius + 1,
                                       2 * increase_radius + 1)))
                img_original.mask = mask_newdata
            # replace possible corrput pixel with zeros for final output
            out = img_original.replaceMaskMedian(box_x,
                                                 box_y,
